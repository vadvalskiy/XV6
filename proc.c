...
wait(void)
{
  ...
  for(;;){
    ...
    havekids = 0;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(LEADER_THREAD(p)->parent != LEADER_THREAD(curproc))
        continue;
      havekids = 1;
    ...
    sleep(LEADER_THREAD(curproc), &ptable.lock);  //DOC: wait-sleep
  }
}
...
tgkill(void) 
{
  ...
  if(LEADER_THREAD(curproc) != curproc){
    return -1;
  }
...
tsuspend(void)
{
    ...
    if(curproc == LEADER_THREAD(curproc)) {
        return -1;
    }
