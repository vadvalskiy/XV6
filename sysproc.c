...
int sys_clone(void) {
    ...
    if(LEADER_THREAD(myproc())->sz < (uint)args) {
        return -1;
    }

    // clone system call supports stack address space which are one page alligned 
    if(LEADER_THREAD(myproc())->sz <= (uint)child_stack && 
       LEADER_THREAD(myproc())->sz < (uint)child_stack - PGSIZE) {
        return -1;
    }
...
sys_sbrk(void)
{
  ...
  acquire(&(LEADER_THREAD(myproc())->tlock));

  addr = LEADER_THREAD(myproc())->sz;
  if(growproc(n) < 0) {
    release(&(LEADER_THREAD(myproc())->tlock));
    return -1;
  }
